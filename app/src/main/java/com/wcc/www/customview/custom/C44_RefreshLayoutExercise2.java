...
public class C44_RefreshLayoutExercise2 extends ViewGroup {
    ...
    public C44_RefreshLayoutExercise2(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void rotateHeaderArrow()
    {
        ...
        ObjectAnimator animator = ObjectAnimator.ofFloat(iv_header_arrow, "rotation",
                rotation, target);
    ...
    private void rotateFooterArrow()
    {
        ...
        float offset = rotation % 180;
        float target = rotation + 180 - offset;
        ObjectAnimator animator = ObjectAnimator.ofFloat(iv_footer_arrow, "rotation",
                rotation, target);
        animator.setDuration(150);
        animator.start();
    }
    ...
    private boolean canChildScrollDown()
    {
        if(Build.VERSION.SDK_INT < 14)
        {
            if(mTarget instanceof AbsListView)
            {
                ...
                return absListView.getChildCount() > 0 && (absListView.getLastVisiblePosition() < absListView.getChildCount() ||
                        absListView.getChildAt(absListView.getChildCount() - 1).getBottom() > mTarget.getMeasuredHeight());
            }else
