 ...
 * 可以在这里做C42_RefreshLayout3上、下拉动被成功触发时的事件处理。
 *
 * Created by 王晨晨 on 2017/10/9.
 */
public class C42_RefreshLayout3 extends ViewGroup{

    private static final String LOG_TAG = C42_RefreshLayout3.class.getSimpleName();
    ...
    public C42_RefreshLayout3(Context context) {
        this(context, null);
    }

    public C42_RefreshLayout3(Context context, AttributeSet attrs) {
        super(context, attrs);

        this.context = context;

        TypedArray array = context.obtainStyledAttributes(attrs,
                R.styleable.C42_RefreshLayout3);
        try {
            pullDownEnabled = array.getBoolean(R.styleable.C42_RefreshLayout3_smart_ui_enable_pull_down, true);
            pullUpEnabled = array.getBoolean(R.styleable.C42_RefreshLayout3_smart_ui_enable_pull_up, true);
            mBackground = array.getDrawable(R.styleable.C42_RefreshLayout3_smart_ui_background);
        } finally {
            array.recycle();
        }
